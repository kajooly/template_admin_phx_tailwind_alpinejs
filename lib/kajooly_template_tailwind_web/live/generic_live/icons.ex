defmodule KajoolyTemplateTailwindWeb.GenericLive.Icons do
  ...
  def icons_chevron_double_right (assigns) do
    ~H"""
      <svg id={assigns[:id]||"icons_chevron_double_right#{gen_id_key()}"} width={assigns[:width]||"16"} height={assigns[:height]||"16"} fill={assigns[:fill]||"currentColor"}  class={" #{assigns[:class]}"} viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
  ...
  def icons_bell (assigns) do
    ~H"""
    <svg xmlns="http://www.w3.org/2000/svg" id={assigns[:id]||"icons_x_octagon#{gen_id_key()}"} width={assigns[:width]||"16"} height={assigns[:height]||"16"} fill={assigns[:fill]||"none"}  class={" #{assigns[:class]}"}  viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
