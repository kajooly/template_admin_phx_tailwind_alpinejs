defmodule KajoolyTemplateTailwindWeb.GenericLive.Tabs do
  ...
  """
    def tabs(assigns) do
     ~H"""
        <div x-data={"{ tab: window.location.hash ? window.location.hash.substring(1).split(',')[0] != undefined ? window.location.hash.substring(1).split(',')[0] : '#{ assigns[:open] }' : '#{ assigns[:open] }', isCompactMode: $persist(false).as('isCompactMode') }"} id={"tab_wrapper_#{gen_id_key()}"} class="mb-4 w-full">
          <%= if @nav != nil do %>
            <%= for item <- @nav do %>
